...
class ImageUtils(
    ...
    fun getFromMediaUri(uri: Uri?): File? {
        ...
        if (SCHEME_FILE == uri.scheme) {
            ...
            try {
                ...
                // Google Drive images
                return getFromMediaUriPfd(contentResolver, uri)
    ...
    private fun getTempFilename(): String {
        val outputFile = File.createTempFile("image", "tmp", cacheDirectory)
        return outputFile.absolutePath
    }

    private fun getFromMediaUriPfd(resolver: ContentResolver, uri: Uri?): File? {
        if (uri == null) return null
